...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        e_vals, e_vecs = scipy.linalg.eigh(cov_matrix)
    ...
    def transform(self, X, n_components):
        ...
        X_centered = X - self.mean
        X_proj = X_centered.dot(self.W_pca[:, :n_components])
    ...
    def reconstruct(self, X_proj):
        ...
        X = X_proj.dot(self.W_pca[:, :n_components].T) + self.mean
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            S_i = np.dot(X_centered.T, X_centered)
            scatter_within += S_i
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            scatter_between += n_i * np.outer(mu_i - mu, mu_i - mu)
