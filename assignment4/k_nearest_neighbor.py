...
def compute_distances(X1, X2):
    ...
    dists = np.sqrt(np.sum(X1**2, axis=1)[:, np.newaxis] + np.sum(X2**2, axis=1) - 2 * np.dot(X1, X2.T))
...
def split_folds(X_train, y_train, num_folds):
    ...
    y_trains = np.zeros((num_folds, training_size), dtype=np.int32)
    X_vals = np.zeros((num_folds, validation_size, X_train.shape[1]))
    y_vals = np.zeros((num_folds, validation_size), dtype=np.int32)

    # YOUR CODE HERE
    # Hint: You can use the numpy array_split function.
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

    X = np.array(np.array_split(X_train, num_folds))
    y = np.array(np.array_split(y_train, num_folds))

    for i in range(num_folds):
        X_trains[i] = X[np.arange(num_folds) != i].reshape(-1, X_trains.shape[-1])
        X_vals[i] = X[i]
        y_trains[i] = y[np.arange(num_folds) != i].reshape(-1, y_trains.shape[-1])
        y_vals[i] = y[i]
